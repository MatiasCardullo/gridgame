...
Planet units use a declarative runtime in `src/core/planet_units.rs`.
...
The base window has `Auto builder units`.
...
The builder window has three auto-selection toggles:
...
- The current format stores preset, behavior, node bindings, path, cargo, fuel, and current action/target
