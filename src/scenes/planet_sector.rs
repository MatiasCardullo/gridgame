...
use crate::core::{
    ...
    add_item, build_requirements, is_under_construction, new_placed_block, AppConfig, Axial,
...
pub fn run(
    ...
    update_units_and_mining(blocks, tiles, units, base_hex, dt);
    ...
    for r in -TRI_LENGHT..=TRI_LENGHT {
        for q in -TRI_LENGHT..=TRI_LENGHT {
            ...
            if let Some(placed) = blocks.get(&hex) {
                ...
                let port_view = false;
                if port_view && !is_under_construction(placed) {
    ...
    if let Some((depot, a, b, kind)) = spawn_request {
        let path = if kind == BuildBlockType::Logistics {
            ...
            if first.is_empty() || second.is_empty() || third.is_empty() {
                ...
                // TODO: Smooth station_in dwell without path hacks.
                if let Some(out_index) = combined.iter().position(|hex| *hex == b) {
